...
typedef struct FuzzTarget {
    ...
    /*
     * Optional per-target custom input mutator. Used by structured fuzzers
     * (e.g. proto-fuzz-* via libprotobuf-mutator) so each target can install
     * its own type-aware mutation. fuzz.c routes libFuzzer's
     * LLVMFuzzerCustomMutator calls through this. NULL => fall back to
     * libFuzzer's default LLVMFuzzerMutate.
     */
