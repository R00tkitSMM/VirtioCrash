/* proto-fuzz-virtio-balloon -- virtio-balloon-device on arm-virt. */
...
#include "proto_fuzz_virtio_balloon.pb.h"
...
constexpr uint32_t kMaxQueues=8, kVqStride=0x3000;
...
uint32_t place_desc(uint32_t vq, uint64_t addr, uint32_t len, bool dw, bool chain) {
    auto& q = g.vqs[vq];
...
void submit_pfn_list(uint32_t vq, const Pkt& m) {
    if (g.vqs[vq].size == 0) return;
    if (g.vqs[vq].next_desc_idx + 1 >= g.vqs[vq].size) return;
    /* Pack each PFN as little-endian u32. */
    std::string buf;
    int n = std::min<int>(m.pfns_size(), 1024);
    buf.reserve(n * 4);
    for (int i = 0; i < n; ++i) {
        uint32_t p = m.pfns(i);
        char b[4]; std::memcpy(b, &p, 4); buf.append(b, 4);
    }
    if (buf.empty()) buf.assign(4, '\0');
    uint32_t id = alloc_buf(buf.data(), (uint32_t)buf.size());
    if (!id) return;
    auto& bb = g.bufs[id];
...
void submit_stats(const qemu::fuzz::balloon::Stats& m) {
    constexpr uint32_t STATS_VQ = 2;
    if (g.vqs[STATS_VQ].size == 0) return;
    if (g.vqs[STATS_VQ].next_desc_idx + 1 >= g.vqs[STATS_VQ].size) return;
    /* virtio_balloon_stat: u16 tag + u64 value */
    std::string buf;
    int n = std::min<int>(m.entries_size(), 64);
    buf.reserve(n * 10);
    for (int i = 0; i < n; ++i) {
        const auto& e = m.entries(i);
        uint16_t t = (uint16_t)e.tag();
        uint64_t v = e.value();
        char b[10];
        std::memcpy(b+0, &t, 2);
        std::memcpy(b+2, &v, 8);
    ...
    if (buf.empty()) buf.assign(10, '\0');
    uint32_t id = alloc_buf(buf.data(), (uint32_t)buf.size());
    if (!id) return;
    auto& bb = g.bufs[id];
    place_desc(STATS_VQ, bb.gpa, bb.len, false, false);
    mw(R_QUEUE_NOTIFY, STATS_VQ);
    flush_events(g.qts);
}

void submit_free_page(const qemu::fuzz::balloon::FreePageCmd& m) {
    constexpr uint32_t FP_VQ = 3;
    if (g.vqs[FP_VQ].size == 0) return;
    if (g.vqs[FP_VQ].next_desc_idx + 1 >= g.vqs[FP_VQ].size) return;
    uint32_t cmd = m.cmd_id();
    char b[4]; std::memcpy(b, &cmd, 4);
    uint32_t id = alloc_buf(b, 4);
    if (!id) return;
    auto& bb = g.bufs[id];
    place_desc(FP_VQ, bb.gpa, bb.len, false, false);
    mw(R_QUEUE_NOTIFY, FP_VQ);
    flush_events(g.qts);
}

void op_mem_write_absolute(const qemu::fuzz::balloon::MemwriteAbsolute& m) {
...
void dispatch(const qemu::fuzz::balloon::VirtioOp& op) {
    using V = qemu::fuzz::balloon::VirtioOp;
    switch (op.op_case()) {
    case V::kGetFeatures:
    ...
    case V::kInflate: submit_pfn_list(0, op.inflate()); break;
    case V::kDeflate: submit_pfn_list(1, op.deflate()); break;
    case V::kStats:   submit_stats(op.stats()); break;
    case V::kFreePage: submit_free_page(op.free_page()); break;
...
void run(QTestState* qts, const unsigned char* data, size_t size) {
    g.qts = qts;
    qemu::fuzz::balloon::VirtioBalloonProgram prog;
...
GString* cmdline(FuzzTarget*) {
    ...
    /* free-page-hint and stats-vq are device-driver-negotiated; we'll
     * just enable the device and let LPM toggle features in SetFeatures. */
    g_string_assign(s,
...
size_t mutator(uint8_t* d, size_t s, size_t max, unsigned int seed) {
    qemu::fuzz::balloon::VirtioBalloonProgram in;
...
static void register_target(void) {
    static const FuzzTarget t = {
        "proto-fuzz-virtio-balloon",
        "LPM-driven structured fuzzer for virtio-balloon-device on arm virt",
...
static void do_qemu_init_register_proto_fuzz_virtio_balloon(void) {
