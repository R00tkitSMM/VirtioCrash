/* proto-fuzz-virtio-console -- virtio-serial-device on arm-virt. */
...
#include "proto_fuzz_virtio_console.pb.h"
...
constexpr uint32_t kMaxQueues=16;
...
/* Port-id to vq mapping for virtio-console:
 *   port 0 rx = vq 0, tx = vq 1
 *   ctrl rx   = vq 2, tx = vq 3
 *   port N rx = vq 2+2*N, tx = vq 3+2*N (N>=1) */
uint32_t port_tx_vq(uint32_t pid) { return pid == 0 ? 1 : (3 + 2 * pid); }
uint32_t port_rx_vq(uint32_t pid) { return pid == 0 ? 0 : (2 + 2 * pid); }
constexpr uint32_t CTRL_RX_VQ = 2;
constexpr uint32_t CTRL_TX_VQ = 3;

void submit_tx(const qemu::fuzz::console::TxData& m) {
    uint32_t vq = port_tx_vq(m.port_id() & 0xf);
    if (vq >= kMaxQueues || g.vqs[vq].size == 0) return;
    if (g.vqs[vq].next_desc_idx + 1 >= g.vqs[vq].size) return;
    size_t n = std::min<size_t>(m.data().size(), 4096);
    uint32_t id = alloc_buf(m.data().data(), (uint32_t)n);
    if (!id) return;
    auto& b = g.bufs[id];
    place_desc(vq, b.gpa, b.len, false, false);
    mw(R_QUEUE_NOTIFY, vq);
    flush_events(g.qts);
}

void submit_rx(const qemu::fuzz::console::RxBuf& m) {
    uint32_t vq = port_rx_vq(m.port_id() & 0xf);
    if (vq >= kMaxQueues || g.vqs[vq].size == 0) return;
    if (g.vqs[vq].next_desc_idx + 1 >= g.vqs[vq].size) return;
    uint32_t sz = m.size() ? m.size() : 1024;
    if (sz > 4096) sz = 4096;
    std::string buf(sz, '\0');
    uint32_t id = alloc_buf(buf.data(), sz);
    if (!id) return;
    auto& b = g.bufs[id];
    place_desc(vq, b.gpa, b.len, true, false);
    mw(R_QUEUE_NOTIFY, vq);
    flush_events(g.qts);
}

void submit_ctrl(const qemu::fuzz::console::CtrlMsg& m) {
    uint32_t vq = CTRL_TX_VQ;
    if (g.vqs[vq].size == 0) return;
    if (g.vqs[vq].next_desc_idx + 1 >= g.vqs[vq].size) return;

    /* virtio_console_control: id(4) event(2) value(2) [extra] */
    std::string out;
    char b[8];
    uint32_t id = m.port_id();
    uint16_t ev = (uint16_t)m.event();
    uint16_t val = (uint16_t)m.value();
    std::memcpy(b+0, &id, 4);
    std::memcpy(b+4, &ev, 2);
    std::memcpy(b+6, &val, 2);
    out.append(b, 8);
    size_t n = std::min<size_t>(m.data().size(), 256);
    if (n) out.append(m.data().data(), n);

    uint32_t bid = alloc_buf(out.data(), (uint32_t)out.size());
    if (!bid) return;
    auto& bb = g.bufs[bid];
    place_desc(vq, bb.gpa, bb.len, false, false);
    mw(R_QUEUE_NOTIFY, vq);
    flush_events(g.qts);
}

void op_mem_write_absolute(const qemu::fuzz::console::MemwriteAbsolute& m) {
...
void dispatch(const qemu::fuzz::console::VirtioOp& op) {
    using V = qemu::fuzz::console::VirtioOp;
    switch (op.op_case()) {
    case V::kGetFeatures:
    ...
    case V::kTxData: submit_tx(op.tx_data()); break;
    case V::kRxBuf:  submit_rx(op.rx_buf()); break;
    case V::kCtrlMsg: submit_ctrl(op.ctrl_msg()); break;
...
void run(QTestState* qts, const unsigned char* data, size_t size) {
    g.qts = qts;
    qemu::fuzz::console::VirtioConsoleProgram prog;
...
GString* cmdline(FuzzTarget*) {
    ...
    /* virtio-serial-device with one extra port (id=1) backed by the
     * "null" chardev. Port 0 is the console, port 1 a normal port. */
    g_string_assign(s,
        "qemu-system-aarch64 -display none -machine virt -nodefaults -m 128 "
        "-global virtio-mmio.force-legacy=false "
        "-device virtio-serial-device,id=vser0,max_ports=4 "
        "-chardev null,id=cons0 "
        "-device virtconsole,chardev=cons0,bus=vser0.0 "
        "-chardev null,id=port1 "
        "-device virtserialport,chardev=port1,bus=vser0.0,nr=1");
    return s;
}

size_t mutator(uint8_t* d, size_t s, size_t max, unsigned int seed) {
    qemu::fuzz::console::VirtioConsoleProgram in;
...
static void register_target(void) {
    static const FuzzTarget t = {
        "proto-fuzz-virtio-console",
        "LPM-driven structured fuzzer for virtio-serial-device on arm virt",
...
static void do_qemu_init_register_proto_fuzz_virtio_console(void) {
