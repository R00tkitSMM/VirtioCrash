...
/*
 * libFuzzer's built-in mutator, exposed so a custom mutator can fall back
 * to it.
 */
extern size_t LLVMFuzzerMutate(uint8_t *data, size_t size, size_t max_size);

/*
 * Called by libFuzzer to produce the next input from the current one. We
 * route through fuzz_target->custom_mutator when set so structured fuzzers
 * (e.g. proto-fuzz-*) can do type-aware mutation. When unset, fall back
 * to libFuzzer's default byte-level mutator.
 */
size_t LLVMFuzzerCustomMutator(uint8_t *data, size_t size, size_t max_size,
                               unsigned int seed)
{
    if (fuzz_target && fuzz_target->custom_mutator) {
        return fuzz_target->custom_mutator(data, size, max_size, seed);
    }
